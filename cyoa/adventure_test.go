...
func Test(t *testing.T) {
	...
	if len(am) != 7 {
		t.Errorf("invalid lenght of adventures map, should be 7, is %v", len(am))
	}
	_, present := am["debate"]
	if !present {
...
func TestRunCli(t *testing.T) {
	...
	actual := underTest[:10]
	if !bytes.Equal(actual, []byte{61, 61, 61, 61, 61, 61, 61, 61, 61, 61}) {
		t.Error("Wrong prefix")
	}

	actual = underTest[4178:4185]
	if !bytes.Equal(actual, []byte{105, 110, 97, 46, 10, 10, 10}) {
		t.Error("Wrong suffix")
	}

	actual = underTest[1968:1974]
	if !bytes.Equal(actual, []byte{32, 102, 101, 119, 32, 116}) {
		t.Errorf("Wrong content in the middle, should be slopes, was|%v|", actual)
	}
}

// All text variables are below for clarity of tests
